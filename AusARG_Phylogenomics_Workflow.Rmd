...
\textcolor{DarkOrchid}{Output:} a */trim_reads* directory with *_adapters.fa*, *_duplicates.fq.gz*, and *_dd.fastq.gz* files. If filtering the reads, the final output files are *_bb.final.fastq.gz*. 
...
+ *file*: the sample info file (annoyingly you need to specify *dir/file*, e.g. frogs/samples.csv)
...
\textcolor{DarkOrchid}{Output:} *_final.fq.gz* and *_unpaired.final.fq.gz* files.
...
\textcolor{DarkOrchid}{Output:} a */phylogeny* directory which holds a *locus_data.csv* summary file and a */alignments* subdirectory holding all the rough locus alignment files (*_.fasta*).
...
\textcolor{DarkOrchid}{Output:} *_NT.fasta* and *_AA.fasta* alignment files are located in respective subdirectories of */macse*. 
...
~SqCL_Pipeline$ python Scripts/align_close_frame_general.py \
                --indir [...] --program [...] --CPU 40
```
**Requirements:**

+ *indir*: directory that holds the alignment files. Your working directory should still be **SqCL_Pipeline**, so *--indir* will bebelow that, e.g. *--indir frogs/[alignment dir]*
+ *program*: options are "refine", "refineLemmon", "export", "align". *refine* and *refineLemmon* work with a good existing alignment and just tweaks it slightly, and so is comparatively fast. *align* assumes the input alignment is raw and needs considerable work, and so is much slower. *export* quickly reads in the file, and spits out a version with closed reading frames, but does little else.
+ *CPU*: number of available cores, doubles as the number of parallel instances to run
...
For general use we've added some scripts that might be useful for handling alignments outside the context of the pipeline, meaning they are not dependent on the directory substructure. They do have to be operated from the SqCL directory as they call the same singularity files that we have stored there.  
If we have combined a series of rough alignments from different projects we may want to quickly trim them with gblocks.


**I NEED TO FIX THE ANNOYING TRAILING / ON THE END OF DIRECTORIES SO IT'S CONSISTENT BETWEEN SCRIPTS**

## *align_gtrim_general*
```{bash eval=F}
~SqCL_Pipeline$ python Scripts/align_gtrim_general.py \
                --indir [...] --CPU 60

e.g. python Scripts/align_gtrim_general.py --indir Crown_Frogs/Alignments/ --CPU 60
```

## *align_close_frame_general*
We may then want to close the reading frames with MACSE (*refine*), or do a more thorough alignment (*align*).
```{bash eval=F}
~SqCL_Pipeline$ python Scripts/align_close_frame_general.py \
                --indir [...] --CPU 60

e.g. python Scripts/align_close_frame_general.py --program align \
     --indir Crown_Frogs/Alignments/clean_gb_alignments --CPU 60
```

## *phylogeny_genetrees_general*
And finally just generate some gene trees with IQTREE
```{bash eval=F}
~SqCL_Pipeline$ python Scripts/phylogeny_genetrees_general.py \
                --indir [...] --CPU [...]
  
e.g. python Scripts/phylogeny_genetrees_general.py \
     --indir Crown_Frogs/Alignments/macse_NT/ --CPU 60
```
